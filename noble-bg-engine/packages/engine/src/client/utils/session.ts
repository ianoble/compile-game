...
export function saveSession(gameId: string, matchID: string, session: PlayerSession): void {
	localStorage.setItem(key(gameId, matchID), JSON.stringify(session));
}

export function loadSession(gameId: string, matchID: string): PlayerSession | null {
